...
旧系统冻结在 main(880b62d);**rebuild 分支按 ADR-0005 重写中**(增量 1-9 已落地——通知落库与多端同步收口见 issue #14,交接见 `docs/handoffs/`)。README.md 已随 `2e9a5b9` 重写为目标态;开发命令、约定与排坑以本文件 + CONTEXT.md 为准。业务决策唯一约束 = @docs/adr/0005-architecture-rebuild-production-charter.md;验收基线 = @docs/acceptance-scenarios.md;术语表(目标态,以它为准)= @CONTEXT.md。旧系统术语/类名只在被取代的决策记录中保留,不得当作现行架构。
