...
# 评测 CLI(离线;run 须 --base-url 指向净库上运行的评测 app,地址由 reset-db 打印;--scenarios 相对仓库根解析;
#          run/score 烧真 token;reset-db 属破坏性操作,须先获显式授权)
cd python-backend && uv run python scripts/evals.py list|validate|reset-db|run|score
```
...
> CI 两条 workflow:`.github/workflows/ci.yml`(push main 与 PR:后端 ruff/ty/快速 pytest,前端 lint/vitest/build);`.github/workflows/gitleaks.yml`(**push/PR 只扫推送范围;每周一 + 手动触发全历史**;误报豁免在 `.gitleaks.toml`,勿扩面)。
...
| `judge_model` / `judge_api_url` / `judge_api_key` | 评测 judge(独立于 `llm_*`:跨厂判分模型,走中转站的 Anthropic 面;URL/KEY 留空或 MODEL 置空即显式报错,不静默降级) |
