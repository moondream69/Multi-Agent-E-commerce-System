...
## 现状变更(2026-09-07,issue #4)

- 前端视图收敛为 驾驶舱/客服工作台/审批中心,买家前台商店/订单视图已移除(见 CONTEXT.md「买家前台旅程」)。
- 主动通知呈现从聊天面板淡黄气泡改为驾驶舱 Agent 卡片铃铛:`chat:notification` 信封新增 `kind` 字段按业务主题归类,未读计数由前端 localStorage 持久化。
