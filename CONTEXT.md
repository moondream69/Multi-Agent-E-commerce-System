...
**演示买家**:
无账号体系下代表买家的固定客户(seed 客户「张伟」,按 email `zhangwei@example.com` 定位);REST 下单绑定到该客户,客服 Agent 的主动通知以他为口吻生成。非多买家体系——只有这一个买家视角。
_Avoid_: 用户、账户、张伟(中文语境可直接说,术语表中可与「演示买家」并列)

**买家前台旅程**:
买家视角的页面闭环:商品商店 → 立即购买(REST 直下 pending 订单)→ 我的订单 → 客服中心(售后聊天)。前端已收敛为 驾驶舱/客服工作台/审批中心 三视图(均面向卖家运营),商店/订单前台视图已移除,买家旅程仅剩后端 REST 下单入口。
_Avoid_: 门店、商城、用户旅程
...
**主动通知**:
客服 Agent 订阅订单状态变更 / 库存告警后,主动生成的通知(买家未说话,客服先来消息)。经 `customer.notification` 事件 → WS `chat:notification` 推送到驾驶舱对应 Agent 卡片的通知铃铛,按信封 `kind` 字段归类(order_status / inventory_alert → 订单卡,未知 kind 兜底客服卡);未读以红色数字徽标累计,前端 localStorage 持久化;同时保留 `agent:event` 全量桥接(后台视角)。
_Avoid_: 推送、弹窗、客服消息
