...
export class ScoringTool implements ITool {
  ...
  calculate(input: ScoringInput): {
    ...
    const priceScore = Math.min(input.avgPrice / 100, 100) * 0.2;
    const marginScore = input.margin * 0.2;
    const growthScore = Math.max(0, input.growthRate + 50) * 0.1;

    const score =
      searchScore + competitionScore + priceScore + marginScore + growthScore;
    const grade =
      score >= 80
        ? 'A (强烈推荐)'
        : score >= 60
