...
export class InventoryAlertTool implements ITool {
  ...
  check(
    ...
    if (ratio <= 0) message = `🔴 ${productName} 已售罄！请立即补货。`;
      ...
      message = `🟠 ${productName} 库存严重不足 (当前: ${currentStock}, 安全线: ${threshold})。建议3天内补货。`;
    else if (ratio < 0.6)
      message = `🟡 ${productName} 库存偏低 (当前: ${currentStock}, 安全线: ${threshold})。建议7天内补货。`;
    else if (ratio < 1)
      message = `🔵 ${productName} 库存接近安全线 (当前: ${currentStock})。关注销量趋势。`;
