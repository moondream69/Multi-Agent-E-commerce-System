...
const FAQ_TOPICS = [
  ...
  },
  {
    topic: '退货与退款',
    count: 15,
    tags: ['returns', 'refunds', 'after-sales'],
  },
  ...
  { topic: '关税与清关', count: 15, tags: ['customs', 'duties', 'tax'] },
  {
    topic: '产品质量与真伪',
    count: 15,
    tags: ['quality', 'authenticity', 'warranty'],
  },
...
export class FaqGenerator {
  ...
  constructor(
    ...
    private readonly repo: Repository<FaqEmbedding>,
      ...
      try {
        const response = await this.llm.complete(
          [
            {
              role: 'system',
              content: '你是跨境电商客服专家。生成高质量的FAQ问答对。',
            },
            {
              role: 'user',
              content: `生成 ${topic.count} 对关于"${topic.topic}"的FAQ JSON数组。
