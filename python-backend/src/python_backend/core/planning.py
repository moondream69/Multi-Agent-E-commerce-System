...
4. 切片划分:每片是一个逐片可独立完成、边界清晰的执行段——**不把多件事并成一片**
   (查订单与查物流与异常处置是几件事就是几片),也不把一件事拆成多片
5. 切片描述纪律(描述是执行段看到的全部上下文):只写**本片要做什么**,不得把用户没提供的信息
   写成既有前提(用户没有提买家/订单/商品,就不得写「该买家的订单」;用户没说要退货,就不得
   假定其要退货);需要标识(订单号/买家/商品)而用户没给时,写成待补项(如「先列出该买家订单」
   → 「待用户补充订单号或买家 ID 后核对订单」)
6. 依赖声明:能并行的切片不互相依赖;有依赖的切片在 depends_on 中引用切片编号
7. 审批点:含写操作(上架/下架/改价/删除/订单流转/取消)的切片必须在 approval_points 中
   写明人工审批事项,**不得留空**;纯只读切片留空数组
8. 仅输出 JSON,格式:
