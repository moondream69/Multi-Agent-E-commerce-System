...
- 引用小点(issue #51 / #67):提示词给**三类证据统一编号**(FAQ/订单/商品,ref 贯穿),
  ...
  citations 随回答一起下发——商品类结论也能标注,不再只有语料条目可核(#71:提示词补
  通用方括号约束,方括号只作引用标记)
- 空正文(issue #65,现由 ``llm.complete_with_blank_retry`` 承担,#68 升为全仓共用):
  ...
  预算问题不再直接变成用户可见的 500
- 无据说明(issue #72,与客服线 ``DRAFT_SYSTEM`` 同口径):未经查证的流程/渠道/政策类
  说明即使是通识也不得写成确定口吻——实评里模型被「怎么查」类问句诱导,用常识补出
  「查询渠道」四条零引用 bullet(库内无据),判据③(无凭空论断)判 0
            ...
            [
                {
                    ...
                    "content": (
                        ...
                        "未经查证的流程/渠道/政策类说明(去哪查、谁来通知、怎么办理),即使是通识,"
                        "也不得写成确定口吻——查证证据里没有的,如实说「未查到相关说明」或直接不提。"
                        "依据查证证据作答时,在该句末尾用方括号标出所依据的证据编号(如 [1]),"
                        "编号取自查证证据的 ref(FAQ 条目、订单、商品均带 ref,皆可标);"
                        "未依据证据的句子不标;方括号只用于引用标记,不要用方括号写状态或备注(如 [done])。"
                        "语气专业友善,只输出回复正文。"
                    ),
                },
                {
