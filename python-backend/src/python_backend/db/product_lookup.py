...
class ProductLookupStore(Protocol):
    """商品定位存储协议(issue #35):操作面 = 按 SKU/标题/类目查询(纯只读)。

    issue #42(A5「按类目查询」):第三分支 category——类目精确匹配(大小写不敏感),
    为「按类目盘货」提供对话侧入口;优先级 sku > title > category(精确 > 模糊 > 枚举)。
    """

    async def lookup(
        self, *, sku: str | None, title: str | None, category: str | None, limit: int
    ) -> tuple[list[dict], bool]:
        """按 SKU(精确,大小写不敏感)、标题(contains,大小写不敏感)或类目(精确,大小写不敏感)查询。
    ...
    async def lookup(
        ...
        async with SessionFactory() as session:
            if sku:
                ...
                statement = select(Product).where(func.lower(Product.category) == category.strip().lower())
            else:
                raise ValueError("product_lookup 需提供 sku、title 或 category 至少其一")
