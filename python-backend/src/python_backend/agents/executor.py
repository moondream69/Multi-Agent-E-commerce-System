...
class ToolExecutor:
    ...
    async def _execute_scoring(self, params: dict) -> dict:
        raw = await complete_with_blank_retry(
            ...
            [
                {
                    # #70:理由只能用材料——缺口维度照实写「无数据」,不许拿行业套话填
                    "role": "system",
                    "content": (
                        "你是选品评分专家。根据**给定材料**从市场潜力、竞争程度、利润率、运营难度四维评分,"
                        '仅输出 JSON:{"score": 0-100 整数, "grade": "A/B/C/D", "rationale": "一句话理由"}。'
                        "rationale 只能陈述给定材料支持的事实;材料里有对应**定性**事实的维度(如竞争格局、"
                        "市场结构、政策方向、采纳障碍)照实评分并说明依据;只有该维度在材料里完全无对应内容时"
                        "才写「该维度无数据,按中性计分」,不得用行业常识或套话填补——缺数据不等于不能给分,"
                        "但编事实会让整份报告自相矛盾"
                    ),
                },
    ...
    async def _execute_generate_report(self, params: dict) -> dict:
        report = await self._llm.complete(
            [
                {
                    "role": "system",
                    # #70:理由面与 _execute_scoring 同一口径——报告层的套话同样会被判「凭空论断」
                    "content": (
                        "你是选品分析师,请基于给定情报生成结构化选品分析报告(含结论与风险)。"
                        "报告里的每条结论与理由只能来自给定情报;情报未覆盖的维度如实标注「无数据」,"
                        "不得用行业常识或套话补齐。**缺口维度按中性计分只是计分口径,不等于对结论的否定**:"
                        "报告仍须给出明确的分级结论与优先级次序(可注明证据薄弱、需补哪些数据),"
                        "不得用「无区分度 / 不构成判断 / 不建议据此排序」这类话把评分本身否掉。"
                    ),
                },
