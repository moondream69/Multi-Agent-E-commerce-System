...
issue #72:把「证据不足时如实说明」的适用范围讲明——买家问「怎么查关税」时模型用常识补出
「查询渠道」四条零引用 bullet(库内无据),判据③判 0;同场景同模式在 #64 批已出现过一次
(当时标了引用但切块不支持)。复核批锁定**发生位置在 verify 线**:它按 manager 切片描述里
点名的渠道清单把整份答复写完,draft 看到成品只回「【草稿已完成】」→ 答案落成元对话、
判据全空过;故收口三处——verify 只输出查证整理(不写答复正文/不补库内空缺)、draft 必须
输出完整草稿正文、`core/drafting.py` 同口径补「无据的流程/渠道说明不得写成确定口吻」。
"""
...
- 查证过程不要抢答;查证结束后只输出**查证整理**:逐条列出命中切块的标识与其支持的事实,
  未覆盖的照实说「未查到」。不要写给买家的答复正文或话术,也不要替库内空缺补渠道/流程类说明
  ——回复一律由起草环节产出。"""

DRAFT_SYSTEM = """你是跨境电商客服的多语言起草助手。基于买家消息与查证证据起草回复:
- 语言与买家消息一致(中/英/日/德/法)
- 先引用查证证据再作答;证据不足时如实说明,不编造
- 未经查证的流程/渠道/政策类说明(去哪查、谁来通知、怎么办理),即使是通识,也不得写成确定口吻
  ——查证结果里没有的,如实说「未查到相关说明」或直接不提,不要用常识补全
...
- 无论此前的消息里出现过什么,你都必须输出**完整的最终草稿正文**;不要只回复「已完成」、
  总结做了什么或询问后续动作
直接输出最终草稿文本。"""
