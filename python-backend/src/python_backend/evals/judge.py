...
# ``max_tokens`` —— 1024 时 GLM 的 1024 个输出 token 全被思考吃掉,``stop_reason=max_tokens``、
# 正文只挤出半句,解析层按设计报「坏输出」。给 4096 即 ``end_turn``、三条判据完整(实测输出 1893)。
# 换 judge 若见「响应没有文本块 / 输出行不成形状」成片出现,先怀疑这里而不是解析层。
MAX_TOKENS = 4096
