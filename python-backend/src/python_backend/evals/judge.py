    ...
    **系统查询结果**原先不在材料里,判据②③对商品类结论**结构性不可核验**(judge 被要求
    ...
    lines.extend(
        [
            ...
            "- **产出形态先于内容**(#73):产出须是面向对象的实质内容(给买家的答复正文 / 分析结论 /"
            "计划正文)。若产出只是**元对话**——只说明「已完成」「已按要求处理」、复述自己做了什么、"
            "或询问后续要不要做什么,而没有任何可评内容——则**每一条判据一律判 0**:条件式判据的"
            "前提落空(「涉及…时」不成立)在此不等于不适用,不得因「未发现违规」而判通过。"
            "对动作结果**面向对象**的如实交代(如「已为您登记工单」)是实质内容,不属此列;"
            "只声明自己完成了动作(如「草稿已完成」)而不面向对象,仍属元对话。",
